...
def runFitAccuracy(model, xTrain, yTrain):
    ...
    accuracy = model.evaluate(xTrain, yTrain, batch_size=memBatchSize)
...
def memoryMain():
    ...
    for stonk in nameList:
        ...
        xTrain, yTrain, _, _, Date = prepareMemoryData(stonk, columnValues)

        model, xTrain, yTrain = runMemory(xTrain, yTrain)

        model, results = runFitAccuracy(model, xTrain, yTrain)
